...
这是 Java 后端 `dev` Profile 的 seed 账号，只能用于本地开发。演示模式下不请求后端，任意非空账号和密码都可以登录。生产构建默认不展示测试凭据；如需在专用测试环境显示，显式设置：

```dotenv
VITE_SHOW_TEST_CREDENTIALS=true
```

正式环境请使用后端分配的管理员账号，不要复用开发密码。
